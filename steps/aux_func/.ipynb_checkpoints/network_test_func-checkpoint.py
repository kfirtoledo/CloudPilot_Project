...
def run_single_iperf_test(src_name, dst_name, dest_ip, dest_port, res_dir, type_data):
    Tests=["bw"]
...
def get_folder_res(host_zone,host_platform,target_zone,target_platform,proxy_zone,proxy_platform):
    time_s = get_time()
    #get folder path
    real_path = os.path.realpath(__file__)
    proj_dir = os.path.dirname(os.path.dirname(os.path.dirname(real_path))) # directory of script
    res_dir= f'{proj_dir}/results/host-{host_zone}_{host_platform}/target-{target_zone}_{target_platform}/proxy-{proxy_zone}_{proxy_platform}/time-{time_s}/'
    if not os.path.exists(res_dir):
        os.makedirs(res_dir)
    return res_dir
# return current time
